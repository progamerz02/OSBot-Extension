const ScriptTableController = (function() {
	...
	function onMessage(e) {
		...
		if (e.source === window) {
			...
			switch (action) {
				case 'sales-users': showSalesUsers(scriptRow); break;
				case 'sales-profit': showSalesProfit(scriptRow); break;
				case 'sales-latest': showSalesLatest(scriptRow); break;
				case 'sales-renewals': showSalesRenewals(scriptRow); break;
				case 'trial-total': showTrialTotal(scriptRow); break;
				case 'trial-active': showTrialActive(scriptRow); break;
				case 'trial-expired': showTrialExpired(scriptRow); break;
				
				case 'all-sales-users': showAllSalesUsers(scriptRow); break;
				case 'all-sales-profit': showAllSalesProfit(scriptRow); break;
				case 'all-sales-latest': showAllSalesLatest(scriptRow); break;
				case 'all-sales-renewals': showAllSalesRenewals(scriptRow); break;
				case 'all-trial-total': showAllTrialTotal(scriptRow); break;
				case 'all-trial-active': showAllTrialActive(scriptRow); break;
				case 'all-trial-expired': showAllTrialExpired(scriptRow); break;
				
				case 'stacktrace': showStacktrace(scriptRow); break;
				
				case 'add-trial': showAddTrial(scriptRow); break;
			}
		}
	}
	...
	function showAddTrial(scriptRow) {
		...
		var trialTime = parseInt(scriptRow.querySelector(`input[name="trial-time-${scriptId}"]`).value || localStorage.getItem(`trial-time-${scriptId}`));
		// Try obtain from field. If doesn't work, try get it from local cache.
		
		tPopup.set(`${scriptName} (${trialTime} Hours)`, 512, 256, tNewTrial.html, (async function(e) {
			var selectedUsers = tNewTrial.getSelectedUsers();
			
			tPopup.setContents();
			if (selectedUsers && selectedUsers.length > 0) {
				for (let i = 0; i < selectedUsers.length; i++) {
					await addTrialAndNotifyUser(scriptRow.dataset, selectedUsers[i], trialTime);
				}
	...
	function showSalesUsers(scriptRow) {
		...
		tPopup.set(scriptName, 650, 650, prepareIFrameChart(tLineChart, {
	...
	function showSalesProfit(scriptRow) {
		...
		var data = scriptRow.dataset.salesRecords;
			...
			data = JSON.stringify(data);
		tPopup.set(scriptName, '100%', 650, prepareIFrameChart(tMaterialBarChart, {
	...
	function showTrialActive(scriptRow) {
		...
		if (trials) {
			...
			preparePopupForTrials(`Active Trials for ${scriptName}`, scriptRow, trials);
			tPopup.show();
		}
	}
	...
	function showTrialExpired(scriptRow) {
		...
		if (trials) {
			trials = trials.filter(trial => trial.expired);
			preparePopupForTrials(`Expired Trials for ${scriptName}`, scriptRow, trials);
			tPopup.show();
		}
	}
	...
	function showAllSalesUsers(scriptRows) {
		...
		var rows = scriptRows.filter(tr => Boolean(tr.dataset.salesRecords))
							 .map(tr => Object.assign({
	...
	function showAllSalesProfit(scriptRows) {
		...
		var rows = scriptRows.filter(tr => Boolean(tr.dataset.salesRecords)).map(tr => Object.assign({
			name: tr.dataset.scriptName,
			data: tr.dataset.salesRecords
		}));
		...
		tPopup.set('All Transactions', '100%', 650, prepareIFrameChart(tMaterialBarChart, {
	...
	function showAllSalesLatest(scriptRows) {
		...
		var data = ChartUtil.generateAllSalesMaterialBarChart(rows, sale => sale.latest);
	...
	function prepareIFrameChart(chartTemplate, data) {
		return `<iframe src="data:text/html;charset=utf-8,${escape(chartTemplate.prepare(data))}" width="${data['chart.width']}" height="${data['chart.height']}"></iframe>`;
	}
	...
	function preparePopupForTrials(title, scriptRow, trials) {
		...
		if (tTrialContainer.validate()) {
			...
			trials.sort((a, b) => (a.userId === ipsSettings.memberID)+(b.userId === ipsSettings.memberID));
